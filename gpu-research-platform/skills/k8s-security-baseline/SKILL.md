...
| 10 | NetworkPolicies in every namespace (5.3.1/5.3.2) | `kubectl get networkpolicy -A` — every tenant ns needs ≥1; verify CNI supports NetworkPolicy (ask Lambda which CNI) | Default-deny ingress (+ egress at L2 maturity) stamped per namespace (researcher-tenancy-provisioner bundle) | High |
