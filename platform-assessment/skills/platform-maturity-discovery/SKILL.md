...
A high-confidence maturity finding requires behavioral evidence plus an independent corroborating source. One source is a lead, not a conclusion. When sources disagree, preserve the contradiction as a finding; do not average it away.
...
|---|---|---|---|
| Communication and knowledge | Email/chat themes, docs, wikis, search logs, support channels | Repeated how-to requests, manual approval patterns, knowledge gaps, shadow workflows, decision discoverability | Productivity from message counts; intent from isolated messages |
...
| Work demand and flow | Jira, Azure Boards, GitHub Issues/Projects, ServiceNow | Queue time, blocked reasons, handoffs, toil share, request recurrence, rework, platform-support demand, agent-linked work | Story points across teams as comparable productivity |
| Code and delivery | Repositories, ownership, PRs, CI/CD, releases, test and policy results | Traceability, automation coverage, review latency, pipeline reliability, golden-path use, security gates, agent attribution | Lines of code or commit volume as value |
| Infrastructure and operations | Cloud inventory, Kubernetes, IaC state, CMDB, observability, incidents, costs | IaC coverage, drift, provisioning lead time, self-service coverage, reliability, fleet currency, cost attribution, manual operations | Maturity from tool presence without adoption or outcomes |
...
| Agent and governance evidence | Agent identities, session logs, tool calls, sandboxes, evals, policy decisions, audit trails | Scoped identity, governed execution, deterministic validation, exception rates, acceptance/rework, shadow-agent usage | Safe autonomy from license adoption or demo success |
...
Before collection, write a discovery charter covering purpose, approved systems, population, time window, fields, retention, access, and intended decisions. Confirm legal, HR, privacy, security, labor/works-council, and data-owner requirements where applicable.
...
3. **Write discovery questions.** For each maturity aspect and ADP gate, state the question, expected evidence, source, calculation, and confidence rule before collecting data.
4. **Collect behavioral evidence.** Prefer first-party, read-only connectors or APIs. Pull work-flow events, repository and CI/CD configuration, deployment and incident events, platform usage, infrastructure/IaC state, cost and reliability data, and approved aggregate communication/meeting metadata.
5. **Collect human evidence.** Use one anonymous practitioner survey plus standardized interviews or sensing sessions. Sample platform users, bypassers, platform staff, security, operations, and leaders. Keep questions consistent across cohorts; separate experience from aspiration.
6. **Normalize and triangulate.** Build the evidence catalog, calculate the core measures, grade provenance and completeness, and create a contradiction register. Investigate material mismatches — for example, fast pipeline telemetry alongside widespread reports of slow delivery.
