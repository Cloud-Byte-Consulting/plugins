...
description: Decide between prompt engineering (zero/few-shot in-context learning), RAG, PEFT/LoRA adapter fine-tuning, and full fine-tuning or continued pretraining for LLM adaptation — a decision framework over data volume, latency, cost, drift rate, and IP constraints — then execute the PEFT workflow (dataset standardization → tokenization → adapter training with LoRA/AdaLoRA/QLoRA → evaluation) and run fine-tunes as Kubernetes Jobs with checkpointing to object storage. Use whenever the user asks "should we fine-tune or use RAG?", prompt vs fine-tune, mentions fine-tuning, LoRA, QLoRA, AdaLoRA, PEFT, adapters, domain adaptation, continued pretraining, instruction tuning, task-specific tuning, few-shot examples, quantized training, bitsandbytes, Hugging Face Trainer, target modules, rank/alpha, ROUGE scores, or evaluating a fine-tuned model; wants a model to speak the org's domain language or match a house style; or asks how to run a fine-tune on the GPU cluster. For multi-GPU/multi-node fine-tunes use the sibling distributed-training-advisor skill; for running sweeps over LoRA hyperparameters use ray-on-k8s-engineer; for logging and registering the resulting adapters use experiment-registry-standard.
---
...
2. **Tokenization.** Base model's tokenizer; truncation + padding to fixed `max_length` (e.g., 512); pad token = EOS; add BOS/EOS; labels = input_ids for CLM. Inspect a few tokenized samples — silent truncation of long answers is a classic data bug.
